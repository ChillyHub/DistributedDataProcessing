...
public class CarPainter extends JPanel implements CarEventsListener {
    ...
    private final static int defaultCellSize = 60;
    private final static int minGap = 24;
    ...
    public CarPainter(FieldMatrix fieldMatrix) {
        ...
        f.setSize(fieldMatrix.cols * defaultCellSize + 2 * minGap,
                fieldMatrix.rows * defaultCellSize + 6 * minGap);
        f.add(this);
    ...
    public void paintComponent(Graphics g) {
        ...
        int verticalGap = (gridHeight - step * fieldMatrix.rows) / 2 + minGap * 3;
        ...
        for (Car car : cars) {
            ...
            g.drawString(String.format("Car [%s] || Color: %s || Move Direction: %s || Move Speed: %s",
                    car.getName(), car.getColor(), car.getDirection(), car.getSpeed()), left, yGap);
            yGap += minGap;

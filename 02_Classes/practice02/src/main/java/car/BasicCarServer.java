...
public class BasicCarServer implements CarServer {
    ...
    @Override
    public boolean moveCarTo(Car car, Position position) {
        Position from = car.getPosition();
        boolean ret = fieldMatrix.moveCarTo(from.row, from.col, position.row, position.col);
        carEventsListener.carMoved(car, from, position, ret);
        return ret;
    }

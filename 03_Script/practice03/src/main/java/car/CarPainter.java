...
public class CarPainter extends JPanel implements CarEventsListener {
    ...
    public void paintComponent(Graphics g) {
        ...
        int gridWidth = screenWidth - (Math.max(2, cars.size() + 1)) * minGap;
        int gridHeight = screenHeight - 2 * minGap;
        int step = Math.min(gridWidth / fieldMatrix.cols, gridHeight / fieldMatrix.rows);

        int verticalGap = (gridHeight - step * fieldMatrix.rows) / 2 + minGap * Math.max(1, cars.size());
        ...
        for (Car car : cars) {
            ...
            g.drawString(String.format("Car [%s] || Move Direction: %s || Move Speed: %s",
                    car.getName(), car.getDirection().toString(), car.getSpeed()), left, yGap);
            yGap += minGap;
